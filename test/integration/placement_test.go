...
var _ = ginkgo.Describe("Placement", func() {
	...
	ginkgo.Context("Scheduling", func() {
		...
		ginkgo.It("Should re-create placementdecisions successfully once placementdecisions are deleted", func() {
			...
			assertCreatingPlacement(placementName, noc(10), 5, "", nil)
		...
		ginkgo.It("Should schedule successfully once spec.NumberOfClusters is increased", func() {
			...
			assertCreatingPlacement(placementName, noc(5), 5, "", nil)
		...
		ginkgo.It("Should create multiple placementdecisions once scheduled", func() {
			...
			assertCreatingPlacement(placementName, nil, 101, "", nil)
		...
		ginkgo.It("Should schedule successfully based on ClusterResourcePreferenceTypeMostAllocatableToCapacityRatio", func() {
			// cluster settings
			clusterNames := []string{
				clusterName + "-1",
				clusterName + "-2",
			...
			clusterResources[1] = []string{"9", "10", "80", "100"}
			clusterResources[2] = []string{"7", "10", "90", "100"}
		...
		ginkgo.It("Should schedule successfully based on ClusterResourcePreferenceTypeMostAllocatable", func() {
			// cluster settings
			clusterNames := []string{
				clusterName + "-1",
				clusterName + "-2",
		...
		ginkgo.It("Should re-schedule successfully once placementdecisions' balance and cluster resource changes", func() {
			// cluster settings
			clusterNames := []string{
				clusterName + "-1",
				clusterName + "-2",
			...
			assertUpdatingClusterWithClusterResources(clusterNames[1], []string{"5", "10", "30", "100"})
